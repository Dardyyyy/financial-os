  ...
  const pickResult = async (r: Result) => {
    ...
    try {
      if (cryptoMode && r.cgId) {
        ...
        if (p != null) { setBuy(String(p)); setPriceMsg(`Aktueller Kurs ${eur2(p)} - uebernommen`); }
            ...
            {!picked ? (
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3 bg-panel2/60 border rounded-2xl px-3 py-3" style={{ borderColor: accent }}>
                  ...
                  <button onClick={unpick} className="text-muted hover:text-bad text-sm shrink-0">andern \u2715</button>
                </div>
