...
type Result = { symbol: string; name: string; cgId?: string };
...
export default function Portfolio() {
  ...
  const rows = useMemo(() => holds.map(h => {
    ...
    return { ...h, valueEur, costEur, pl, plPct: costEur > 0 ? (pl / costEur) * 100 : 0 };
  }), [holds, fx]);
  ...
  const doSearch = async () => {
    ...
    try {
      const r = await fetch(`/api/search?q=${encodeURIComponent(q.trim())}${cryptoMode ? "&type=crypto" : ""}`).then(x => x.json());
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-5">
        <div className="card p-6">
          ...
          {/* Hinzufuegen via Suche (Aktie ODER Krypto) */}
          <div className="mt-6 border-t border-line/50 pt-5 space-y-3">
            <div className="flex gap-2">
              <button onClick={() => switchMode(false)} className="chip" style={!cryptoMode ? { borderColor: "#F5B544", color: "#F5B544" } : {}}>Aktie suchen</button>
              <button onClick={() => switchMode(true)} className="chip" style={cryptoMode ? { borderColor: "#5EEAD4", color: "#5EEAD4" } : {}}>Krypto suchen</button>
            </div>
